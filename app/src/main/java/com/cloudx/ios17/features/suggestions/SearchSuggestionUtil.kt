...
class SearchSuggestionUtil {
    fun getSuggestionProvider(context: Context): com.cloudx.ios17.features.suggestions.SuggestionProvider {
        val defaultSearchEngine = defaultSearchEngine(context)

        return with(defaultSearchEngine) {
            when {
                contains(Provider.QWANT.key, true) -> com.cloudx.ios17.features.suggestions.QwantProvider()
                else -> com.cloudx.ios17.features.suggestions.DuckDuckGoProvider()
            }
        }
    }
